...
- **Validação em funções dedicadas e explícitas** (`validar_contrato`, `validar_amortizacoes`, `validar_taxa_indexador`), não espalhada inline pelo meio do cálculo — cada uma levanta mensagens específicas e compreensíveis para o usuário final, nunca uma mensagem genérica de exceção Python.
